...
const ADMIN_HASH = "$2a$10$r6SgSAn9C.0fGq7p9C8CLe1XfG4u9I.5v0r4hI.4R2vO0tO6W6C6C"; // пароль spinogrz666

app.use((req, res, next) => {
    const ip = req.headers['x-forwarded-for'] || req.socket.remoteAddress;
    req.userIP = ip;
    if (bannedIPs.has(ip)) return res.status(403).json({ error: "YOU ARE BANNED" });
    next();
});

app.get('/', (req, res) => res.send("MORIX API IS RUNNING"));
